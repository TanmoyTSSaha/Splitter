...
class AchievementService {
  ...
  Future<List<AchievementModel>> fetchUnlocksInMonth(
    String userId,
    DateTime month,
  ) async {
    try {
      ...
      final rows = await _supabase
          .from(SupabaseTables.userAchievements)
          .select(
            '${SupabaseColumns.unlockedAt}, '
            'achievements!inner(${SupabaseColumns.id}, ${SupabaseColumns.slug}, '
            '${SupabaseColumns.name}, ${SupabaseColumns.description}, ${SupabaseColumns.iconKey})',
          )
          .eq(SupabaseColumns.userId, userId)
          .gte(SupabaseColumns.unlockedAt, monthStart.toIso8601String())
          .lte(SupabaseColumns.unlockedAt, monthEnd.toIso8601String())
          .order(SupabaseColumns.unlockedAt, ascending: false);
      ...
      AppErrorReporter.unexpected(
        'AchievementService.fetchUnlocksInMonth failed',
        error: e,
        stack: stack,
        context: {'feature': 'achievements'},
      );
