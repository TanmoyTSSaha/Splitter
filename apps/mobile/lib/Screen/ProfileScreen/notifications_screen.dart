...
import 'package:splitr/Services/SupabaseServices/push_preferences_service.dart';
...
const Map<String, IconData> _typeIcons = {
  ...
  NotificationTypes.loanRequest: Icons.handshake_outlined,
  NotificationTypes.settlementReminder: Icons.notifications_active_outlined,
  NotificationTypes.budgetAlert: Icons.savings_outlined,
...
const Map<String, Color> _typeColors = {
  ...
  NotificationTypes.loanRequest: NotificationTypeColors.loanRequest,
  NotificationTypes.settlementReminder: NotificationTypeColors.settlementReminder,
  NotificationTypes.budgetAlert: NotificationTypeColors.budgetAlert,
...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  PushPreferences _pushPrefs = PushPreferences.defaults();
  bool _loadingPushPrefs = true;
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      if (!_loadingPushPrefs) ...[
                        _buildPushPrefsCard(context),
                        const SizedBox(height: 16),
                      ],
  ...
  Widget _buildPushPrefsCard(BuildContext context) {
    final surface = Theme.of(context).colorScheme.surface;
    final borderColor = groupMutedBorderHairline;

    SwitchListTile pushSwitch({
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Padding(
            padding: const EdgeInsets.only(top: groupGapSm, bottom: groupGapSm),
            child: Text(
              AppStrings.notifications.pushAlertsTitle,
              style: TextStyle(
                fontFamily: kFontPoppins,
                fontSize: splitrFontBody,
          ...
          Text(
            AppStrings.notifications.pushAlertsSubtitle,
            style: TextStyle(
              fontFamily: kFontPoppins,
              fontSize: splitrFontCaption,
              color: groupOnSurfaceMuted,
          ...
          const SizedBox(height: groupGapSm),
          pushSwitch(
            title: AppStrings.notifications.pushFriendRequest,
            value: _pushPrefs.friendRequest,
            onChanged: (v) => _updatePushPref((p) => p.copyWith(friendRequest: v)),
          ),
          pushSwitch(
            title: AppStrings.notifications.pushGroupInvite,
            value: _pushPrefs.groupInvite,
            onChanged: (v) => _updatePushPref((p) => p.copyWith(groupInvite: v)),
          ),
          pushSwitch(
            title: AppStrings.notifications.pushExpenseAdded,
            value: _pushPrefs.expenseAdded,
            onChanged: (v) => _updatePushPref((p) => p.copyWith(expenseAdded: v)),
          ),
          pushSwitch(
            title: AppStrings.notifications.pushSettlement,
            value: _pushPrefs.settlement,
            onChanged: (v) => _updatePushPref((p) => p.copyWith(settlement: v)),
          ),
          pushSwitch(
            title: AppStrings.notifications.pushSettlementReminder,
            value: _pushPrefs.settlementReminder,
            onChanged: (v) =>
                _updatePushPref((p) => p.copyWith(settlementReminder: v)),
          ),
          pushSwitch(
            title: AppStrings.notifications.pushLoanRequest,
            value: _pushPrefs.loanRequest,
            onChanged: (v) => _updatePushPref((p) => p.copyWith(loanRequest: v)),
          ),
          pushSwitch(
            title: AppStrings.notifications.pushBudgetAlert,
            value: _pushPrefs.budgetAlert,
            onChanged: (v) => _updatePushPref((p) => p.copyWith(budgetAlert: v)),
          ),
          pushSwitch(
            title: AppStrings.notifications.pushMarketing,
            subtitle: AppStrings.notifications.pushMarketingSubtitle,
            value: _pushPrefs.marketing,
            onChanged: (v) => _updatePushPref((p) => p.copyWith(marketing: v)),
          ),
        ],
      ),
    );
